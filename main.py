...
class TrasferimentoForm(ui.Modal, title="🔄 Form Trasferimento Operatore"):
    ...
    async def on_submit(self, interaction: Interaction):
        canale = interaction.client.get_channel(899561903448260628)
        emoji_qualifica = trova_emoji(self.qualifica_operatore.value, interaction.guild.emojis)
        messaggio = f"> **{self.qualifica_operatore.value}** {emoji_qualifica} {self.utente.mention} viene *trasferito* presso il **{self.reparto_trasferimento.value.upper()}** a seguito dell'approvazione della richiesta di trasferimento vista necessità di personale all'interno del reparto.\n\n*Autorizzato da: {interaction.user.mention}*"
        try:
            embed_dm = discord.Embed(
                title="🔄 Trasferimento Eseguito",
                description=f"> **{self.qualifica_operatore.value}** {emoji_qualifica} sei stato *trasferito* presso il reparto **{self.reparto_trasferimento.value.upper()}**.\n\nMotivo: necessità di personale nel reparto.",
            ...
            pass
        await canale.send(messaggio)
        await interaction.response.send_message("✅ Trasferimento comunicato con successo!", ephemeral=True)
